<template>
    <div class="cart">
        <div class="cart-header">
            <div class="cart-header-title">購物清單</div>
            <div class="cart-header-main">
                <div class="cart-info">商品訊息</div>
                <div class="cart-price">單價</div>
                <div class="cart-count">數量</div>
                <div class="cart-cost">小計</div>
                <div class="cart-delete">移除</div>
            </div>
        </div>
        <div class="cart-content">
            <div class="cart-content-main" v-for="(item, index) in cartList">
                ...
                <div class="cart-delete">
                    <span class="cart-control-delete" @click="handleDelete(index)">移除</span>
                </div>
            </div>
            <div class="cart-empty" v-if="!cartList.length">購物車為空</div>
        </div>
        <div class="cart-promotion" v-show="cartList.length">
            <span>使用優惠碼：</span>
            <input type="text" v-model="promotionCode">
            <span class="cart-control-promotion" @click="handleCheckCode">驗證</span>
        </div>
        <div class="cart-footer" v-show="cartList.length">
            <div class="cart-footer-desc">
                共計 <span>{{ countAll }}</span> 件商品
            </div>
            <div class="cart-footer-desc">
                應付總額 <span>￥ {{ costAll - promotion }}</span>
                <br>
                <template v-if="promotion">
                    （優惠 <span>￥ {{ promotion }}</span>）
                </template>
            </div>
            <div class="cart-footer-desc">
                <div class="cart-control-order" @click="handleOrder">現在結算</div>
            </div>
        </div>
    </div>
</template>
